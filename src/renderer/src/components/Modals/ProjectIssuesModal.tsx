...
export default function ProjectIssuesModal({ issues, onClose }: Props): React.ReactElement {
  ...
  const hasAutoRemaining = fixable.some(
    (i) => i.repair === 'auto' && states[i.id]?.status === 'pending'
  )

  return (
    <div className="create-modal-backdrop" onClick={onClose}>
      <div className="create-modal issues-modal" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="issues-modal-head">
          ...
          <button className="issues-modal-close" onClick={onClose}>
            ×
          </button>
        </div>
        ...
        <div className="issues-modal-foot">
          <span className="issues-foot-stat">
            {fixedCount > 0 && (
              <>
                <span className="issues-foot-num">{fixedCount}</span> fixed
              </>
            )}
            {fixedCount > 0 && remaining > 0 && ' · '}
            {remaining > 0 && (
              <>
                <span className="issues-foot-num">{remaining}</span> remaining
              </>
            )}
            {fixedCount === 0 && remaining === 0 && (
              <span style={{ color: 'var(--color-ink-60)' }}>All issues resolved</span>
            )}
          </span>
  ...
  return (
    <div className={`issue-row ${state.status}`}>
      ...
      <div className="issue-row-action">
        {state.status === 'working' && <span className="issue-status working">Working…</span>}
        ...
        {state.status === 'error' && (
          <span className="issue-status error" title={state.errorMessage}>
            Failed
          </span>
        )}

        {state.status === 'pending' && issue.repair === 'auto' && (
          <button className="issue-fix-btn" onClick={onRunAuto}>
            Fix
          </button>
        )}

        {state.status === 'pending' && issue.repair === 'text' && !state.textExpanded && (
          <button className="issue-fix-btn" onClick={() => onPatch({ textExpanded: true })}>
            Set description…
          </button>
        )}
